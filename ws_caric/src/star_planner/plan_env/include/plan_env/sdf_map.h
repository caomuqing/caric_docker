...
public:
  ...
  void inputMap(const pcl::PointCloud<pcl::PointXYZ> &points, const int &point_num, bool is_occupied,const Eigen::Vector3d &slf_pose);
  void updateSlfPose(const Eigen::Vector3d& pose);
  ...
  std::vector<Eigen::Vector3i> getFrtCandidateCells();
...
struct MapData {
  ...
  std::vector<Eigen::Vector3i> frt_candidate_cells_;
...
inline std::vector<Eigen::Vector3i> SDFMap::getFrtCandidateCells() {
  return md_->frt_candidate_cells_;
}
