...
namespace fast_planner
{
  ...
  void SDFMap::initMap(ros::NodeHandle &nh)
  {
    ...
    bbx_->cal_global_map(box_min, box_max);
    mp_->box_mind_ = box_min;
    mp_->box_maxd_ = box_max;
    mp_->box_mind_[2] = 0.0;
      ...
      const Eigen::Vector3d &camera_pos)
  {
    ...
    for (int i = 0; i < point_num; ++i)
    {
      ...
      double step = 0.1;
      Eigen::Vector3d start_pos = camera_pos;
      Eigen::Vector3d end_pos = pt_w;
      Eigen::Vector3i start_id, end_id;
      posToIndex(start_pos, start_id);
      posToIndex(end_pos, end_id);
      Eigen::Vector3d dir = (end_pos - start_pos).normalized();
      Eigen::Vector3d now_pos = start_pos;
      Eigen::Vector3i now_id = start_id;
      // ROS_ERROR("start pos(%f %f %f) -> end(%f %f %f)", start_pos(0), start_pos(1), start_pos(2), end_pos(0), end_pos(1), end_pos(2));
      // ROS_ERROR("start id (%d %d %d) -> end(%d %d %d)", start_id(0), start_id(1), start_id(2), end_id(0), end_id(1), end_id(2));
      while ((now_pos - end_pos).norm() > step / 2.0)
      {
        // ROS_ERROR("now_pos %f %f %f", now_pos(0), now_pos(1), now_pos(2));
        // ROS_ERROR("now_id  %d %d %d", now_id(0), now_id(1), now_id(2));
        setCacheOccupancy(toAddress(now_id), 0);
        now_pos = now_pos + step * dir;
        posToIndex(now_pos, now_id);
      }
    }
  ...
  void SDFMap::inputMap(const pcl::PointCloud<pcl::PointXYZ> &points, const int &point_num, bool is_occupied, const Eigen::Vector3d &slf_pose)
  {
    if (point_num == 0)
      return;
    ...
    Eigen::Vector3d update_min(points.points[0].x, points.points[0].y, points.points[0].z);
    Eigen::Vector3d update_max(points.points[0].x, points.points[0].y, points.points[0].z);
    Eigen::Vector3d box_min, box_max;
    getBox(box_min, box_max);
    for (int k = 0; k < 3; ++k)
    {
      update_min[k] = max(update_min[k], box_min[k]);
      update_max[k] = min(update_max[k], box_max[k]);
    }
    if (md_->reset_updated_box_)
    {
    ...
    if (!is_occupied)
    {
      md_->frt_candidate_cells_.clear();
    }
    Eigen::Vector3i slf_id;
    posToIndex(slf_pose, slf_id);
    for (int i = 0; i < point_num; ++i)
    {
      auto &pt = points.points[i];
      pt_w << pt.x, pt.y, pt.z;

      if (!bbx_->isInAnyBoundingBox(pt_w) && is_occupied)
        continue;

      posToIndex(pt_w, idx);
      if (idx == slf_id && is_occupied)
        continue;
      ...
      if (is_occupied)
        md_->occupancy_buffer_[vox_adr] = mp_->min_occupancy_log_ + 0.1;
      else
      {
        md_->occupancy_buffer_[vox_adr] = mp_->clamp_min_log_ + 0.1;
        if (isNeighborOccupied(idx))
          md_->frt_candidate_cells_.push_back(idx);
      }
    }
  ...
  inline bool SDFMap::isNeighborOccupied(const Eigen::Vector3i &voxel)
  {
  ...
  inline vector<Eigen::Vector3i> SDFMap::sixNeighbors(const Eigen::Vector3i &voxel)
  {
  ...
  void SDFMap::updateSlfPose(const Eigen::Vector3d &pose)
  {
    Eigen::Vector3i idx;
    posToIndex(pose, idx);
    vector<Eigen::Vector3i> nbs = allNeighbors(idx);
    for (auto id : nbs)
    {
      int vox_adr = toAddress(id);
      md_->occupancy_buffer_[vox_adr] = mp_->clamp_min_log_ + 0.1;
    }
  }

  inline vector<Eigen::Vector3i> SDFMap::allNeighbors(const Eigen::Vector3i &voxel)
  {
