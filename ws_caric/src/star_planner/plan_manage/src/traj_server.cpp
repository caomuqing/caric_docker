...
ros::Publisher cmd_vis_pub, pos_cmd_pub, traj_pub, pitch_cmd_pub;
nav_msgs::Odometry odom;
quadrotor_msgs::PositionCommand cmd;
geometry_msgs::Twist pitch_cmd;

// Info of generated traj
vector<NonUniformBspline> traj_, traj_pitch_;
...
ros::Time start_time, end_time, last_time, start_time_pitch_;
double energy;
...
void bsplineCallback(const bspline::BsplineConstPtr &msg)
{
  ...
  for (int i = 0; i < msg->pitch_pts.size(); ++i)
    pitch_pts(i, 0) = msg->pitch_pts[i];
  NonUniformBspline pitch_traj(pitch_pts, 3, msg->pitch_dt);
  ...
  {
    traj_pitch_.clear();
    traj_pitch_.push_back(pitch_traj);
    traj_pitch_.push_back(pitch_traj.getDerivative());
  }
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  double yaw, yawdot, pitch, pitchdot;

  // ROS_ERROR("hhhhhhhhhh -> need_pitch_ %d",need_pitch_);
  if (t_cur < traj_duration_ && t_cur >= 0.0)
  {
    ...
    if (need_pitch_)
    {
      pitch = traj_pitch_[0].evaluateDeBoorT(t_cur)[0];
      pitchdot = traj_pitch_[1].evaluateDeBoorT(t_cur)[0];
    }
  }
  else if (t_cur >= traj_duration_)
  {
    ...
    if (need_pitch_)
    {
      pitch = traj_pitch_[0].evaluateDeBoorT(traj_duration_)[0];
      pitchdot = 0.0;
    }
    // Report info of the whole flight
    double len = calcPathLength(traj_cmd_);
    double flight_t = (end_time - start_time).toSec();
    ROS_WARN_THROTTLE(2, "%s flight time: %lf, path length: %lf, mean vel: %lf, energy is: % lf ",
                      drone_name_.c_str(),flight_t, len, len / flight_t, energy);
  }
  else
  {
  ...
  if (need_pitch_)
  {
    pitch_cmd.linear.x = 1.0;
    pitch_cmd.linear.y = -pitch;
    pitch_cmd.linear.z = 0.0;
    pitch_cmd.angular.x = 0.0;
    pitch_cmd.angular.y = 0.0;
    pitch_cmd.angular.z = 0.0;
    pitch_cmd_pub.publish(pitch_cmd);
  }

  // ROS_ERROR_THROTTLE(1.0, "[Traj server] pitch cmd publish pitch:%f pitchdot:%f", pitch, pitchdot);
...
int main(int argc, char **argv)
{
  ...
  pitch_cmd_pub = node.advertise<geometry_msgs::Twist>("command/gimbal", 50);
  ...
  nh.param("exploration/drone_name", drone_name_, string("null"));
