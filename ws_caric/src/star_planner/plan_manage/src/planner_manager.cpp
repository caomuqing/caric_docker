...
void FastPlannerManager::planPitchExplore(const Eigen::Vector3d& start_pitch, const double& end_pitch) {
  ...
  std::cout << "dt_yaw: " << dt_pitch << ", start yaw: " << start_pitch3d.transpose()
            << ", end: " << end_pitch << std::endl;

  while (start_pitch3d[0] < -M_PI/2) start_pitch3d[0] += M_PI;
  while (start_pitch3d[0] >  M_PI/2) start_pitch3d[0] -= M_PI;
  double last_pitch = start_pitch3d[0];

  // Yaw traj control points
  Eigen::MatrixXd pitch(seg_num + 3, 1);
  pitch.setZero();
  ...
  // calcNextYaw(last_pitch, end_pitch3d(0));
  pitch.block<3, 1>(seg_num, 0) = states2pts * end_pitch3d;

  // Debug rapid change of yaw
  if (fabs(start_pitch3d[0] - end_pitch3d[0]) >= M_PI/2) {
    ROS_ERROR("pitch change rapidly!");
    std::cout << "start pitch: " << start_pitch3d[0] << ", " << end_pitch3d[0] << std::endl;
  }
